...
function generateDependencyInfo(
  ...
  const dependentTypes = new Map<
    string,
    { path: string; code: string; isExternal: boolean }
  >()
  ...
  for (const libraryInfo of thirdPartyLibraries) {
    ...
    if (!dependentTypes.has(className)) {
      console.log(
        `[JSDoc] Loading documentation for ${className} from package ${packageName}...`,
      )
      ...
      if (jsdocInfo) {
        ...
        console.log(
          `[JSDoc] Successfully loaded ${className} documentation from ${packageName} index`,
        )
      } else {
        console.log(
          `[JSDoc] No indexed documentation found for ${packageName}, using fallback for ${className}`,
        )
...
export function extractThirdPartyLibraries(
  ...
  while ((match = importRegex.exec(codeContext)) !== null) {
    ...
    if (
      ...
      if (
        defaultImport &&
        packageName &&
        !isBuiltInType(defaultImport) &&
        !isProjectType(defaultImport)
      ) {
      ...
      if (
        namespaceImport &&
        packageName &&
        !isBuiltInType(namespaceImport) &&
        !isProjectType(namespaceImport)
      ) {
  ...
  for (const className of usedClasses) {
    ...
    if (packageName) {
      ...
      const kebabCasePackageName = className.replace(
        /([A-Z])/g,
        (match, letter, index) =>
...
function isProjectType(typeName: string): boolean {
  ...
  return (
    projectSuffixes.some((suffix) => typeName.endsWith(suffix)) ||
    typeName === 'User' ||
    typeName === 'DB'
...
export function generatePrompt(
  ...
  if (!interfaceName) {
    throw new Error(
      'Cannot generate implementation for an anonymous class or interface.',
    )
  }
