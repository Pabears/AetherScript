...
export class CommandServiceImpl extends CommandService {
    ...
    async runJSDocIndex(): Promise<void> {
        ...
        const providerManager = new ProviderManager(); // This might need to be injected via providerFactoryService
    ...
    async runTestProvider(providerName?: string): Promise<void> {
    ...
    async runShowProviderExamples(): Promise<void> {
    ...
    async runTestGeneration(providerName?: string, model?: string): Promise<void> {
        const providerManager = new ProviderManager();
        providerManager.loadFromEnvironment();

        try {
            const { provider, config } = providerManager.createProvider(providerName);
            const testModel = model || config.defaultModel || 'qwen3-coder';
