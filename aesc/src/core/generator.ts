...
} from '../generation/post-processor'
import {
  generateContainer,
  saveGeneratedFile,
    ...
    try {
      ...
      if (classes.length > 0) {
        ...
        if (implClass) {
          ...
          let propertyDeps: PropertyDependency[] = []
      ...
      console.warn(
        `Warning: Could not analyze existing service file ${file}: ${error}`,
      )
    }
  }
...
async function generateSingleService(
  ...
  try {
    ...
    if (fs.existsSync(implFilePath) && !options.force) {
      console.log(
        `  -> SKIPPED: ${implFilePath} already exists. Use --force to overwrite.`,
      )
      return {
        interfaceName,
        status: 'skipped',
    ...
    // Step 4: Code Clear
    const cleanedCode = cleanGeneratedCode(
      rawResponse,
      interfaceName,
      options.verbose,
    )
    ...
    const { constructorDeps, propertyDeps } = Node.isClassDeclaration(
      declaration,
    )
    ...
    const fileDuration = Date.now() - fileStartTime
    console.log(
      `  -> ✅ ${interfaceName} completed in ${(fileDuration / 1000).toFixed(2)}s`,
    )
    ...
    console.error(
      `  -> ❌ Failed to generate ${interfaceName}: ${errorMessage}`,
    )
    return {
      interfaceName,
      status: 'error',
...
async function ensureJSDocIndex(verbose: boolean = false): Promise<void> {
  ...
  const hasExistingIndex =
    fs.existsSync(jsdocDir) &&
    fs.readdirSync(jsdocDir).filter((f) => f.endsWith('.json')).length > 0

  if (hasExistingIndex) {
    if (verbose) {
      console.log(
        '[JSDoc Auto] JSDoc index already exists, skipping auto-generation',
      )
    }
    return
  }
  ...
  try {
    ...
    if (indexedLibraries.length > 0) {
      console.log(
        `✅ JSDoc auto-generation completed! Indexed ${indexedLibraries.length} libraries.`,
      )
      if (verbose) {
    ...
    console.warn(
      '⚠️  JSDoc auto-generation failed:',
      error instanceof Error ? error.message : 'Unknown error',
    )
...
export async function generateCode(
  ...
  console.log(
    `🚀 Starting code generation at ${new Date().toLocaleTimeString()}`,
  )
  ...
  console.log(
    `🚀 Starting concurrent generation of ${servicesToGenerate.size} service(s)...`,
  )

  // Create concurrent generation tasks
  const generationTasks = Array.from(servicesToGenerate.entries()).map(
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      console.error(
        `  -> ❌ Failed to generate ${interfaceName || 'Unknown'}: ${errorMessage}`,
      )
      fileStats.push({
  ...
  const skipCount = fileStats.filter(
    (f) => f.status === 'skipped' || f.status === 'locked',
  ).length
  ...
  if (successCount > 0) {
    ...
    console.log(
      `   ⚡ Average generation time: ${(avgDuration / 1000).toFixed(2)}s`,
    )
  }
  ...
  const allServices = await getAllExistingServices(
    outputDir,
    project,
    generatedServices,
  )
  ...
  if (allServices.length > 0) {
    ...
    console.log(
      `DI container generated successfully with ${allServices.length} services.`,
    )
  } else {
