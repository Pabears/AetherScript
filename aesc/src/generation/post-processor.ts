...
export function postProcessGeneratedCode(
  ...
  console.log(
    `  -> INFO: Found ${originalImports.length} import declarations in original file`,
  )
  ...
  originalImports.forEach((importDecl) => {
    ...
    if (moduleSpecifier.startsWith('.')) {
      ...
      importPath = path
        .relative(path.dirname(generatedFilePath), absoluteImportPath)
        .replace(/\\/g, '/')
    ...
    namedImports.forEach((namedImport) => {
      ...
      if (isTypeOnly) {
        ...
        console.log(
          `  -> INFO: Preserving type-only import 'type ${importName}' from '${importPath}'`,
        )
      } else {
        namedImportsMap.get(importPath)!.add(importName)
        console.log(
          `  -> INFO: Preserving import '${importName}' from '${importPath}'`,
        )
      }
    })
    ...
    if (defaultImport) {
      ...
      console.log(
        `  -> INFO: Preserving default import '${importName}' from '${importPath}'`,
      )
    }
    ...
    if (namespaceImport) {
      ...
      console.log(
        `  -> INFO: Preserving namespace import '${importName}' from '${importPath}'`,
      )
    }
  })
  ...
  const addImport = (type: import('ts-morph').Type) => {
    ...
    for (const decl of symbol.getDeclarations()) {
      ...
      if (
        depSourceFile.isFromExternalLibrary() ||
        depSourceFile.isDeclarationFile()
      )
        continue

      const importPath = path
        .relative(path.dirname(generatedFilePath), depSourceFile.getFilePath())
        .replace(/\\/g, '/')
        .replace(/\.ts$/, '')
  ...
  if (implClass) {
    ...
    typePatterns.forEach((pattern) => {
      ...
      while ((match = pattern.exec(implText)) !== null) {
        ...
        if (typeName && /^[A-Z]/.test(typeName)) {
          // Only consider capitalized names
    ...
    console.log(
      `  -> INFO: Found potential types in implementation: ${Array.from(potentialTypes).join(', ')}`,
    )
    console.log(
      `  -> INFO: Found potential functions in implementation: ${Array.from(potentialFunctions).join(', ')}`,
    )
    ...
    potentialFunctions.forEach((functionName) => {
      ...
      if (!found) {
        for (const [
          importPath,
          namespaceImportName,
        ] of namespaceImportsMap.entries()) {
      ...
      if (!found) {
        ...
        originalSourceFile
          ...
          .forEach((importDecl: import('ts-morph').ImportDeclaration) => {
            const moduleSpecifier = importDecl.getModuleSpecifierValue()
                  ...
                  namedImportsMap
                    ...
                    .add(
                      importName === aliasName
                        ? importName
                        : `${importName} as ${aliasName}`,
                    )
                  console.log(
                    `  -> INFO: Adding missing function import for '${functionName}' from '${importPath}'`,
                  )
                  found = true
                }
              },
            ...
            if (defaultImport && defaultImport.getText() === functionName) {
              ...
              console.log(
                `  -> INFO: Adding missing default function import for '${functionName}' from '${importPath}'`,
              )
              found = true
            }
          })
      ...
      if (!found) {
        console.log(
          `  -> WARNING: Could not find import for function '${functionName}' in original file`,
        )
      }
    })
  }
  ...
  sourceFile
    .getImportDeclarations()
    .forEach((importDecl) => importDecl.remove())
  ...
  for (const [
    ...
    console.log(
      `  -> INFO: Generated default import: import ${defaultImportName} from "${moduleSpecifier}";`,
    )
  }
  ...
  for (const [
    ...
    console.log(
      `  -> INFO: Generated namespace import: import * as ${namespaceImportName} from "${moduleSpecifier}";`,
    )
  }
  ...
  for (const [moduleSpecifier, names] of namedImportsMap.entries()) {
    if (names.size > 0) {
      ...
      console.log(
        `  -> INFO: Generated named imports: import { ${Array.from(names).join(', ')} } from "${moduleSpecifier}";`,
      )
    }
  }
  ...
  for (const [moduleSpecifier, names] of typeOnlyImports.entries()) {
    if (names.size > 0) {
      sourceFile.insertImportDeclaration(0, {
        moduleSpecifier,
        namedImports: Array.from(names).map((name) => ({
          name,
          isTypeOnly: true,
        })),
      })
  ...
  if (Node.isClassDeclaration(declaration) && declaration.isAbstract()) {
    ...
    const baseProperties = new Set(
      declaration.getProperties().map((p) => p.getName()),
    )
    implClass.getProperties().forEach((prop) => {
      if (baseProperties.has(prop.getName())) {
        console.log(
          `  -> INFO: Removing redeclared property '${prop.getName()}' from '${implName}'`,
        )
        prop.remove()
      }
    })
...
export async function validateGeneratedCode(
  ...
  const tempSourceFile = project.createSourceFile(generatedFilePath, code, {
    overwrite: true,
  })
  ...
  try {
    ...
    const flattenMessages = (
      chain: import('ts-morph').DiagnosticMessageChain,
    ): string => {
