  ...
  public formatForLLM(jsdocInfo: JSDocInfo): string {
    ...
    if (jsdocInfo.constructors.length > 0) {
      for (const constructor of jsdocInfo.constructors) {
        ...
        const paramStrings = constructor.parameters.map(
          (p) =>
    ...
    for (const method of jsdocInfo.methods) {
      if (
        method.description ||
        method.parameters.some((p) => p.description) ||
      ...
      lines.push(
        `    ${method.name}(${paramStrings.join(', ')}): ${returnType};`,
      )
    }
  ...
  public generateUsageExample(jsdocInfo: JSDocInfo): string {
    ...
    if (jsdocInfo.constructors.length > 0) {
      ...
      if (constructor) {
        ...
        lines.push(
          `const instance = new ${jsdocInfo.name}(${exampleParams.join(', ')});`,
        )
      }
    } else {
    ...
    const publicMethods = jsdocInfo.methods.filter(
    ...
    if (publicMethods.length > 0) {
      ...
      if (method) {
        ...
        if (method.returnType.includes('Promise')) {
          lines.push(
            `const result = await instance.${method.name}(${exampleParams.join(', ')});`,
          )
        } else {
          lines.push(
            `const result = instance.${method.name}(${exampleParams.join(', ')});`,
          )
        }
      }
    }
