...
3.  **骨架创建 (一次一个服务)**:
    ...
    -   **创建实现**: 现在你可以直接使用 `/aesc-gen` (依赖于 `gemini-cli`)，它能根据 `// @autogen` 标记自动以零幻觉的方式在 `src/generated` 目录下创建该服务的具体实现类 (`...ServiceImpl.ts`)。
...
2.  **编写测试 (一次一个服务)**:
    -   为每个服务创建一个对应的测试文件。现在你可以直接运行 `/aesc-test` 从 `abstract class` 或 `interface` 自动倒推并生成纯净边界测试！
